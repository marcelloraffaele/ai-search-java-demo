...
public class SearchIndexManager {
    ...
    public SearchIndexManager(SearchIndexClient searchIndexClient, String searchIndexName, SearchClient searchClient, TextEmbeddingsService embeddingsService) {
    ...
    public void createIndex() {
        ...
        try {
           index = searchIndexClient.getIndex(searchIndexName);
        }catch(HttpResponseException httpEx) {
            if (httpEx.getResponse().getStatusCode() == 404) 
                logger.info("index {} does not exist. Creating..", searchIndexName);
        }

        if ( index != null) {
            logger.info("index {} already exists. Skipping creation", searchIndexName);
            return;
        }
        ...
        index = new SearchIndex(searchIndexName, fields);
        ...
        logger.info("Created index {}", searchIndexName);
    }
